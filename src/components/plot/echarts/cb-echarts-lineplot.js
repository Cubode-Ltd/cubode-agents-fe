...
template.innerHTML = `
    <div class="cb-echart-lineplot" style="width:100%; height:400px; overflow: hidden;">
...
class LinePlot extends HTMLElement {
  ...
  render() {
    ...
    const option = {
      ...
      xAxis: {
        type: "category",
        data: this.data_.map((point) => point[0]),
      },
      yAxis: {
        type: "value",
      },
      series: [
        {
          type: "line",
          data: this.data_.map((point) => point[1]),
          lineStyle: {
            color: scale(this.data_.length - 1),
          },
        },
      ],
