...
// Display the right categories in x and y
// I'm not understanding if the coordinates works properly
...
class HeatMapPlot extends HTMLElement {
  ...
  plotData(
    ...
    function getUniqueValues(arr) {
      ...
      const filteredArray = flattenedArray.filter(item => item !== null && item !==undefined);
  ...
  updateOption() {
    ...
    while (true) {
      ...
      seriesData.push(plotData.finalData);
      if (index === 0) {
        xAxisData.push(...plotData.xAxisData);
      ...
      // seriesData.push(plotData.finalData);
      // if (index === 0) {
      //   if (Array.isArray(plotData.x_Values,)) {
      //     x_Values.push(...plotData.x_Values,);
      //   }
      //   if (Array.isArray(plotData.y_Values,)) {
      //     y_Values.push(...plotData.y_Values,);
      //   }
      // }
      // index++;
