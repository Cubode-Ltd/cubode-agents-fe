...
import FormComponent from "/media/veracrypt1/cubode-agents-fe/src/components/react/FormComponents.js";
...
class PiePlot extends HTMLElement {
  constructor() {
    ...
    // Select the id to inject the react component
  ...
  // Define the title colorscale and title attribute to be changed 
  static get observedAttributes() {
    return ["colorscale", "color1", "color2","title"];
  }
  ...
  showModal() {
    ...
    //bind the renderForm to the showmodal
    this.renderForm();
  }
  ...
  // import select and inhject the component Form ( is in a different file)
  renderForm() {
    const formContainer = this.shadowRoot.querySelector("#form-container");
    ReactDOM.render(<FormComponent onSubmit={this.handleFormSubmit.bind(this)} />, formContainer);
  }

  // define the submission of the form
  handleFormSubmit({ formData }) {
    console.log("Form data submitted:", formData);
    this.setAttribute("colorscale", formData.colorScale);
    this.setAttribute("title", formData.title);
    this.render();
    this.closeModal();
  }
  ...
  render() {
    ...
    const title = this.getAttribute("title") || "wuuuua";
    let scale;
    ...
    const coloredData = this.data_.map((value, index) => ({
      ...
      name: `Category ${index + 1}`, // Add names to each data point for labels
    }));
  
    

    // Core part of the plot where i pass the title and colorscale( transformed in colorData) 
